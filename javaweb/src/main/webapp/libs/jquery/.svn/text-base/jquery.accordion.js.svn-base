...
(function($) {
				...
				accordion : function(options, data) {
					...
					return this.each(function() {
								if (typeof options == "string") {
									var accordion = $
											.data(this, "ui-accordion");
									accordion[options].apply(accordion, args);
									// INIT with optional options
								} else if (!$(this).is(".ui-accordion"))
									$.data(this, "ui-accordion",
											new $.ui.accordion(this, options));
							});
				},
	...
	function clickHandler(event) {
		...
		// JiPing 修改:
		// 取该header的内容div,如果内容为空
		// 就不做任何事,不打开
		if (clicked.next("div").length > 0) {
			if (clicked.next("div").text().replace(/(^\s*)|(\s*$)/g, "").length == 0) {
				return;
			}
		}
		...
		animations : {
			slide : function(options, additions) {
				...
				var hideHeight = options.toHide.height(), showHeight = options.toShow
						.height(), difference = showHeight / hideHeight;
				options.toShow.css({
							height : 0,
							overflow : 'hidden'
						}).show();
				options.toHide.filter(":hidden").each(options.complete).end()
						.filter(":visible").animate({
									height : "hide"
								}, {
									step : function(now) {
